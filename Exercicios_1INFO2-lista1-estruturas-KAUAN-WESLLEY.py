...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    cigarros = cigarros_fumados_por_dia * 10

    dp = anos_fumando * 365

    min = cigarros * dp

    dias_perdidos_por_fumar =  min / 1440
...
def salario(valor_hora, horas_mensais):
    ...
    bruto = valor_hora * horas_mensais

    porcen = (bruto / 100) * 24
    
    salario = bruto - porcen
...
def imprime_mes_por_extenso(data):
    ...
    como_ficou = (dia + ' de ' + mes_extenso[int(mes)] + ' de '+ ano)
    return como_ficou
