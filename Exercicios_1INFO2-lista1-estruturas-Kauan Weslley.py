...
def metro_para_milimetros(metros):
    ...
    return round(metro_para_milimetros, 2)
...
def tempo_para_percorrer_uma_distancia(distancia, velocidade):
    ...
    tempo_para_percorrer_uma_distancia = distancia / velocidade
    return round(tempo_para_percorrer_uma_distancia,2)
...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    percentual_desconto = preco_original * (percentual_desconto / 100)
    novo_preco = preco_original - percentual_desconto
...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    dias_para_segundos = d + h + m + segundos
    return dias_para_segundos

def celsius_para_fahrenheit(c):

    """ Recebe uma temperatura
    em fahrenheit"""
...
def fahrenheit_para_celsius(f):
    ...
    fahrenheit_para_celsius = (f - 32) * 5/9
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    dp = minutos/1440
    dias_perdidos_por_fumar = dp * dias
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    """ Recebe as notas das 2 provas e 2 exercícios de programação e retorna se o aluno foi ou não a
    aprovado. as provas tem peso 7 e os exercicios tem peso 3. cada parcial têm peso igual"""
    nota1 = p1 * 0.7
    nota2 = p2 * 0.7
    ex1 = ep1 * 0.3
    ex2 = ep2 * 0.3
    media = (nota1 + nota2 + ex1 + ex2)/2
    if media >=  7:
        return True
    elif media < 7:
        return False
...
def salario(valor_hora, horas_mensais):
    ...
    salario = horas_mensais * valor_hora
    s1 = salario * 0.08
    s2 = salario * 0.05
    s3 = salario * 0.11
    salario_todo = salario - (s1 + s2 + s3)
    return salario_todo
