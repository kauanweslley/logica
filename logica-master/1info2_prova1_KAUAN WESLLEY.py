...
def aluguel_airbnb(valor_diaria, dias):
    ...
    aluguel1 = valor_diaria * dias
    limpeza = aluguel1 * 5/100
    aluguel_airbnb = aluguel1 + limpeza + 75
    return round(aluguel_airbnb, 2)
...
def media_final_aprovado_reprovado(prova, exercicio, projeto):
    ...
    notas = ((prova * 4) + (exercicio * 1) + (projeto * 3)) / 8
    return notas >= 7
...
def troca_caixa(texto):
    ...
    texto = texto.replace('r', 'R').replace('q', 'Q').replace('A', 'a').replace('c', 'C').replace('d', 'D').replace('s', 'S').replace('x', 'X').replace('l', 'L')
    return texto
...
def encontra_caracter(texto, caracter_procurado):
    ...
    texto = texto.find(caracter_procurado,0)
    return texto
...
def insere(texto, posicao, texto_inserir):
    ...
    """
    texto = texto[0:posicao] + texto_inserir + texto[posicao:]
    return texto
